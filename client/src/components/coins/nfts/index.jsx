...
import { useIsMountedRef, sleep/*, nfts*/, numberOptimizeDecimal } from '../../../utils';

const Nfts = props => {
	const pageSize = 100;
	const categoryId = 'non-fungible-tokens-nft';
	...
	useEffect(() => {
		const getData = async () => {
			...
			for (let i = 0; i <= marketPage; i++) {
				try {
					// for (let j = 0; j < Math.ceil(nfts.length / (pageSize / 2)); j++) {
					// 	await sleep(i === 0 && j === 0 ? 0 : 500);
					// 	let coinsData = await getCoinsMarkets({ vs_currency: currency, ids: _.slice(nfts, j * (pageSize / 2), (j + 1) * (pageSize / 2)).filter(c => c).join(','), order: 'market_cap_desc', per_page: pageSize / 2, page: i + 1, price_change_percentage: '1h,24h,7d,30d' });
							...
							<CardHeader className="bg-transparent pt-2 pb-4 px-0">
								<Row className="px-0 px-lg-3 mx-0 mx-lg-1">
										...
										<Nav className="nav-pills nav-primary d-flex align-items-center">
											{menus[0].subMenu[1].subMenu.filter(m => m.preset).map((m, key) => (
												<NavItem key={key} style={{ maxWidth: width <= 575 ? 'fit-content' : '' }}>
													<div className={`nav-link${m.path === '/coins/nfts' ? ' active' : ''}${width <= 1200 ? ' f-12 p-2' : ''}`}>
									...
									<Col xl="6" lg="6" md="5" xs="8" className={`mt-3 mt-md-0 d-flex align-items-top ${width <= 575 ? '' : 'justify-content-center'}`}>
										<h1 className="mb-0">
											<div className={`${width <= 575 ? 'f-14' : width <= 991 ? 'f-18' : width <= 1200 ? 'f-20' : 'f-24'} mb-2`} style={{ lineHeight: '1.25' }}>{"Top NFTs Coins"}</div>
