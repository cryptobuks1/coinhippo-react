		...
		const getData = async () => {
			...
			for (let i = 0; i <= marketPage; i++) {
				try {
					...
					let coinsData = await getCoinsMarkets({ vs_currency: currency, order: 'volume_desc', per_page: pageSize / 2, page: i + 1, sparkline: !true, price_change_percentage: '1h,24h,7d,30d' });
					coinsData = coinsData && !coinsData.error ? coinsData : null;
					if (coinsData) {
						const filteredCoinsData = coinsData.filter(c => typeof c.market_cap_rank === 'number');
						for (let j = 0; j < filteredCoinsData.length; j++) {
							newData[size] = filteredCoinsData[j];
							size++;
							if (size === pageSize) {
								break;
							}
						}
						if (coinsData.length < (pageSize / 2) || size === pageSize) {
							if (isMountedRef.current) {
								...
								<Row className="px-0 px-lg-3 mx-0 mx-lg-1">
										...
										<Nav className="nav-pills nav-primary d-flex align-items-center">
											{menus[0].subMenu[0][0].subMenu.map((m, key) => (
												<NavItem key={key} style={{ maxWidth: width <= 575 ? 'fit-content' : '' }}>
													<div className={`nav-link${m.path === '/coins/high-volume' ? ' active' : ''}${width <= 991 ? ' f-12 p-2' : ''}`}>
									...
									<Col xl="6" lg="6" md="6" xs="8" className={`mt-3 mt-md-0 d-flex align-items-top ${width <= 575 ? '' : 'justify-content-center'}`}>
										<h1 className="mb-0">
											...
											<div className={`f-w-300 text-info f-${width <= 575 ? 10 : 14} text-${width <= 575 ? 'left mt-2' : 'center'}`} style={{ lineHeight: 1.5 }}>{"by High Volume"}</div>
										</h1>
									</Col>
