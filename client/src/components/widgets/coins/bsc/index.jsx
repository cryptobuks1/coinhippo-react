...
import { useIsMountedRef/*, sleep, bsc*/, getLocationData, numberOptimizeDecimal } from '../../../../utils';
...
const Bsc = props => {
	...
	useEffect(() => {
		const getData = async () => {
			...
			let size = 0;
			try {
				// for (let j = 0; j < Math.ceil(bsc.length / pageSize); j++) {
				// 	await sleep(j === 0 ? 0 : 500);
				// 	let coinsData = await getCoinsMarkets({ vs_currency: currency, ids: _.slice(bsc, j * pageSize, (j + 1) * pageSize).filter(c => c).join(','), order: 'market_cap_desc', per_page: pageSize, page: 1, price_change_percentage: '24h' });
				...
				// 	}
				// }
				let data = await getCoinsMarkets({ vs_currency: currency, category: 'binance-smart-chain', order: 'market_cap_desc', per_page: n || pageSize, page: 1, price_change_percentage: '24h' });
												...
												<Col lg="12" sm="12" xs="12">
													<Card className="h-100 border-0 mb-0" style={{ boxShadow: 'none' }}>
														<CardHeader className="top-10-card-header d-flex align-items-center pt-3 pb-2 px-3">
															<h2 className="f-16"><a href="/coins/bsc" target="_blank" rel="noopener noreferrer" className="d-flex align-items-center" style={{ color: 'unset', letterSpacing: 0 }}>{"Top "}{n}<img src="https://assets.coingecko.com/markets/images/52/small/binance.jpg" alt="BSC" className="mx-2" style={{ width: '1.5rem' }} />{" BSC by Market Cap"}</a></h2>
															<a href="/coins/bsc" target="_blank" rel="noopener noreferrer" className="ml-auto"><Tooltip title="See more"><MoreHorizontal /></Tooltip></a>
														</CardHeader>
