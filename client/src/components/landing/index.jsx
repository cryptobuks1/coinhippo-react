...
import { useIsMountedRef, sleep, affiliateData, cex, dex/*, nfts, bsc, polkadot*/, getName, numberOptimizeDecimal } from '../../utils';
...
const Landing = props => {
	...
	useEffect(() => {
		const getData = async () => {
			...
			let size = 0;
			try {
				// for (let j = 0; j < Math.ceil(nfts.length / (pageSize * 5)); j++) {
				// 	await sleep(j === 0 ? 0 : 500);
				// 	let coinsData = await getCoinsMarkets({ vs_currency: currency, ids: _.slice(nfts, j * (pageSize * 5), (j + 1) * (pageSize * 5)).filter(c => c).join(','), order: 'market_cap_desc', per_page: pageSize * 5, page: 1, sparkline: true, price_change_percentage: '24h' });
	...
	useEffect(() => {
		const getData = async () => {
			...
			let size = 0;
			try {
				// for (let j = 0; j < Math.ceil(bsc.length / (pageSize * 5)); j++) {
				// 	await sleep(j === 0 ? 0 : 500);
				// 	let coinsData = await getCoinsMarkets({ vs_currency: currency, ids: _.slice(bsc, j * (pageSize * 5), (j + 1) * (pageSize * 5)).filter(c => c).join(','), order: 'market_cap_desc', per_page: pageSize * 5, page: 1, sparkline: true, price_change_percentage: '24h' });
				// 	coinsData = coinsData && !coinsData.error ? coinsData : null;
				// 	if (coinsData) {
				// 		const filteredCoinsData = coinsData.filter(c => true || typeof c.market_cap_rank === 'number');
				// 		for (let k = 0; k < filteredCoinsData.length; k++) {
	...
	useEffect(() => {
		const getData = async () => {
			...
			let size = 0;
			try {
				// for (let j = 0; j < Math.ceil(polkadot.length / (pageSize * 5)); j++) {
				// 	await sleep(j === 0 ? 0 : 500);
				// 	let coinsData = await getCoinsMarkets({ vs_currency: currency, ids: _.slice(polkadot, j * (pageSize * 5), (j + 1) * (pageSize * 5)).filter(c => c).join(','), order: 'market_cap_desc', per_page: pageSize * 5, page: 1, sparkline: true, price_change_percentage: '24h' });
				// 	coinsData = coinsData && !coinsData.error ? coinsData : null;
				// 	if (coinsData) {
				// 		const filteredCoinsData = coinsData.filter(c => true || typeof c.market_cap_rank === 'number');
				// 		for (let k = 0; k < filteredCoinsData.length; k++) {
