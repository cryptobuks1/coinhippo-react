...
const Derivatives = props => {
      ...
      try {
        ...
        if (derivativesData) {
          for (let i = 0; i < derivativesData.length; i++) {
            if (derivativesData[i].contract_type === props.derivativeType) {
  ...
  }, [isMountedRef, props, currency, data]);
                ...
                <Row className="px-0 px-lg-3 mx-0 mx-lg-1">
                  ...
                  <Col lg="6" md="6" xs="8" className={`mt-3 mt-md-0 d-flex align-items-center ${width <= 575 ? '' : 'justify-content-center'}`}>
                    <h1 className="mb-0">
                      ...
                      <div className={`f-w-300 text-info f-${width <= 575 ? 10 : 14} text-${width <= 575 ? 'left mt-2' : 'center'}`} style={{ lineHeight: 1.5 }}>{props.title}{" Contract by Open Interest"}</div>
                    </h1>
                  </Col>
