                                  ...
                                  <tbody>
                                    {sortedTickers.filter((ticker, i) => i >= tablePage * pageSize && i < (tablePage + 1) * pageSize).map((ticker, i) => (
                                      <tr key={i}>
                                        ...
                                        {isDerivative && (
                                          <td className={`text-right ${marketSort.field === 'index' ? 'bg-light' : ''}`}>{typeof ticker.index === 'number' ? numberOptimizeDecimal(numeral(ticker.index).format(ticker.index > 1 ? '0,0.00' : '0,0.0000000000')) : 'N/A'} {ticker.target}</td>
                                        )}
                                        {isDerivative && (
                                        ...
                                        {isDerivative ?
                                          <td className={`text-right ${marketSort.field === 'open_interest_usd' ? 'bg-light' : ''}`}>
                                            ...
                                            {currency && currency.toLowerCase() !== 'usd' && exchangeRatesData['usd'] && typeof exchangeRatesData['usd'].value === 'number' && exchangeRatesData[currency] && typeof exchangeRatesData[currency].value === 'number' && (<div className="f-10 text-info">{currencyData && currencyData.symbol}{numberOptimizeDecimal(numeral(Number(ticker.open_interest_usd) * exchangeRatesData[currency].value / exchangeRatesData['usd'].value).format(Number(ticker.open_interest_usd) * exchangeRatesData[currency].value / exchangeRatesData['usd'].value > 1 ? '0,0.00' : '0,0.0000000000'))}{!(currencyData && currencyData.symbol) && (<>&nbsp;{currency.toUpperCase()}</>)}</div>)}
                                          </td>
                                          :
                                        ...
                                        <td className={`text-right ${marketSort.field === `converted_volume.${currencyMarket}` ? 'bg-light' : ''}`}>
                                          ...
                                          {ticker.target && currencyMarket && ticker.target.toLowerCase() !== currencyMarket.toLowerCase() && (typeof ticker.volume === 'number' || typeof ticker.h24_volume === 'number') && (<div className="f-10 text-info" style={{ wordBreak: 'break-all' }}>{numberOptimizeDecimal(numeral((ticker.volume || ticker.h24_volume) * ticker.last).format('0,0'))}{ticker.target && (<>&nbsp;{ticker.target.startsWith('0X') && ticker.target_coin_id && allCryptoData && allCryptoData.coins && allCryptoData.coins.findIndex(c => c.id === ticker.target_coin_id) > -1 ? allCryptoData.coins[allCryptoData.coins.findIndex(c => c.id === ticker.target_coin_id)].name : ticker.target.toUpperCase()}</>)}</div>)}
                                        </td>
